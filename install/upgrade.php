...
function wp_install( $blog_title, $user_name, $user_email, $public, $deprecated = '', $user_password = '' ) {
	...
	update_option('home', $guessurl);//[ysd] home为固定值
	...
	//[ysd] 初始化数据
   if(is_file(ABSPATH.'wp-admin/install_default_data'))	wp_install_defaults($user_id);
...
function wp_install_defaults($user_id) {
	...
	if ( global_terms_enabled() ) {
		...
		$cat_id = rand(1,1000);//[ysd] 增大主键，防止于导出数据的主键重复
	}
